...
def calc_precision(clusters, ground_truth_labels):
    ...
    tp = numpy.sum(numpy.logical_and(
        coclustering_matrix == gt_coclustering_matrix,
        coclustering_matrix == 1))
...
def calc_accuracy(clusters, ground_truth_labels):
    ...
    conf_matrix = numpy.zeros(
        (num_unique_ground_truth_labels, num_unique_clusters))
    for i in range(num_unique_ground_truth_labels):
        for j in range(num_unique_clusters):
            conf_matrix[i][j] = len([_i_ for _i_ in range(len(clusters))
                                     if clusters[_i_] == unique_clusters[j] and
                                     ground_truth_labels[_i_] ==
                                     unique_ground_truth_labels[i]])

    errors = 0
    row_idx = 0
    for j in numpy.argmax(conf_matrix, axis=1):
        errors += len([_i_ for _i_ in range(len(clusters))
                       if ground_truth_labels[_i_] ==
                       unique_ground_truth_labels[row_idx]])
